...
resource "aws_iam_role" "app_runner_instance_role" {
  name = "PRIME-app-runner-instance-role"
...
resource "aws_iam_role_policy" "app_runner_instance_bedrock_access" {
  name = "PRIME-app-runner-instance-bedrock-policy"
    ...
    image_repository {
      ...
      image_configuration {
        ...
        runtime_environment_variables = {
          ...
          PRIME_API_ENDPOINT = var.prime_api_endpoint
          PRIME_API_KEY      = var.prime_api_key
        }
      }
...
resource "aws_iam_role" "eventbridge_role" {
  ...
  name  = "PRIME-eventbridge-scheduler-role"
...
resource "aws_lambda_function" "scheduler_lambda" {
  count         = var.scheduler_enabled ? 1 : 0
  function_name = "PRIME-researcher-scheduler"
...
resource "aws_iam_role" "lambda_scheduler_role" {
  count = var.scheduler_enabled ? 1 : 0
  name  = "PRIME-scheduler-lambda-role"
