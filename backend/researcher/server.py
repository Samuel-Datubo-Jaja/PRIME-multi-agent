"""
Prime Researcher Service - Investment Advice Agent
"""
...
app = FastAPI(title="Prime Researcher Service")
...
# Configure Bedrock model with US West 2 region
    # Set ALL region environment variables to ensure us-WEST-2 is used gpt-oss-120b 
    ...
    with trace("Researcher"):
        async with create_playwright_mcp_server(timeout_seconds=60) as playwright_mcp:
            agent = Agent(
                name="Prime Investment Researcher",
...
async def health():
    ...
    return {
        "service": "Prime Researcher",
        "status": "healthy",
        "prime_api_configured": bool(os.getenv("PRIME_API_ENDPOINT") and os.getenv("PRIME_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
