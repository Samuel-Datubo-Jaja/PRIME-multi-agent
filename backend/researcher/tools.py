"""
Tools for the PRIME Researcher agent
"""
import os
...
PRIME_API_ENDPOINT = os.getenv("PRIME_API_ENDPOINT")
...
def _ingest(document: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with httpx.Client() as client:
        response = client.post(
            PRIME_API_ENDPOINT,
            json=document,
            headers={"x-api-key": PRIME_API_KEY},
            timeout=30.0
        )
...
def ingest_financial_document(topic: str, analysis: str) -> Dict[str, Any]:
    """
    Ingest a financial document into the PRIME knowledge base.
    ...
    """
    if not PRIME_API_ENDPOINT or not PRIME_API_KEY:
        return {
            "success": False,
            "error": "PRIME API not configured. Running in local mode."
        }
